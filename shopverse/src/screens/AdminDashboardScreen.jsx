...
export default function AdminDashboardScreen() {
  ...
  useEffect(() => {
    const fetchSummaryMetrics = async () => {
      try {
        ...
        // 🚨 THE FIX: Added your exact backend local port address!
        const { data } = await axios.get('http://localhost:5000/api/orders/summary', config); 
        
        console.log("✅ LIVE DATA SECURED:", data); // Open F12 console to see this!
        setSummaryData(data);
        setLoading(false);
      } catch (error) {
        // This will print the exact reason if it fails again
        console.error('❌ DASHBOARD API ERROR:', error.response?.data?.message || error.message);
        setLoading(false);
      }
    };
    fetchSummaryMetrics();
  ...
  // Safely extract data so the screen never crashes
  ...
  return (
    <div className="space-y-6 animate-fade-in p-1 mt-4">
      
      {/* 🧹 THE FIX: I removed the duplicate "Admin Control Panel" title block here */}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className={`p-5 rounded-2xl border shadow-sm flex flex-col justify-between ${
          ...
          <div className="flex items-center justify-between">
            <div>
              <p className={`text-xs font-bold uppercase tracking-wider ${lowStockCount > 0 ? 'text-amber-700 dark:text-amber-500' : 'text-emerald-700 dark:text-emerald-500'}`}>
                Inventory Status
              </p>
              <h3 className={`text-2xl font-black mt-1 ${lowStockCount > 0 ? 'text-amber-900 dark:text-amber-400' : 'text-emerald-900 dark:text-emerald-400'}`}>
                {lowStockCount > 0 ? `${lowStockCount} Low Stock` : 'All Safe'}
              </h3>
            </div>
            <div className={`h-10 w-10 rounded-full flex items-center justify-center ${lowStockCount > 0 ? 'bg-amber-200/50 text-amber-700 dark:bg-amber-500/20 dark:text-amber-400' : 'bg-emerald-200/50 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400'}`}>
      ...
      <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl border border-gray-100 dark:border-gray-700 shadow-sm w-full">
        ...
        {/* 🛠️ THE FIX: Wrapped chart in a hardcoded height container so it CANNOT collapse */}
